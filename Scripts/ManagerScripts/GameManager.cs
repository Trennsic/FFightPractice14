...
public class GameManager : MonoBehaviour
{
    #region // Definitions
    #region // Game Size
    [SerializeField] private float arenaWidth = 400f;
    [SerializeField] private float arenaHeight = 400f;
    [SerializeField] private float gameHeight = 1000f;
    [SerializeField] private float gameWidth = 450f;
    #endregion
    ...
    [SerializeField] private WaymarkManager waymarkManager;
    ...
    #endregion
    [SerializeField] private float rKeyHoldTime = 0f;
    ...
    void InitializeManagers()
    {
        ...
        GetPositionManager().InitializePositionManager();
        GetWaymarkManager().InitializeWaymarkManager();
        GetBackgroundManager().InitializeBackgroundManager();

        //Vector3 circlePos = GetArenaPositionFromPercentage(50, 50);
        //Color orange = new Color(1F, 0.5F, 0.0F);
        //GetDamageMarkerManager().CreateCircleMarker(circlePos, 35f, Color.yellow, .5f, 2f, orange, 1f);
        //
        //Vector3 conePos = GetArenaPositionFromPercentage(70, 50);
        //float coneAngle = 0f;
        //Color purple = new Color(1F, 0F, .8F);
        //GetDamageMarkerManager().CreateConeMarker(conePos, coneAngle, 45f, 80f, Color.magenta, 0.5f, 2f, purple, 1f);
        //
        //Vector3 rectPos = GetArenaPositionFromPercentage(30, 50);
        //GetDamageMarkerManager().CreateRectangleMarker(rectPos, 30f, 40f, Color.cyan, 0.5f, 2f, Color.blue, 1f);
        //
        //Vector3 donutPos = GetArenaPositionFromPercentage(10, 50);
        //Color lime = new Color(.5f, 1f, 0f);
        //GetDamageMarkerManager().CreateDonutMarker(donutPos, 25f, 40f, lime, 0.5f, 2f, Color.green, 1f);
        //
        //Vector3 conePos2 = GetArenaPositionFromPercentage(90, 50);
        //float coneAngle2 = 225f;
        //GetDamageMarkerManager().CreateConeMarker(conePos2, coneAngle2, 45f, 80f, Color.grey, 0.5f, 2f, Color.black, 1f);

    }
    private void CheckForInput()
    {
       if (GetInputManager().GetActionPressed())
        {
            // Progress to next step/attack in fight
            GetFightManager().ProgressFightStep();
        ...
        if (Input.GetKey(KeyCode.R))
        {
            ...
            if (rKeyHoldTime >= rKeyHoldThreshold)
            {
                Debug.Log("R held long enough – restarting game.");
                RestartGame();
                rKeyHoldTime = 0f; // reset so it doesn't keep firing
            }
        }
        else
        {
            rKeyHoldTime = 0f; // reset when R is released
        }
    ...
    public float GetXAsArenaPercentage(float percentage)
    {
        // Normalize the percentage to be between -1 and 1
        float normalizedPercentage = (percentage / 100f) * 2f - 1f;

        // Multiply by half the arena width to scale it to the desired range
        return normalizedPercentage * (GetArenaWidth() / 2f);
    }

    // Convert percentage to a position based on the arena height.
    public float GetYAsArenaPercentage(float percentage)
    {
        // Normalize the percentage to be between -1 and 1
        float normalizedPercentage = (percentage / 100f) * 2f - 1f;

        // Multiply by half the arena height to scale it to the desired range
        return normalizedPercentage * (GetArenaHeight() / 2f);
    }
    public Vector3 GetArenaPositionFromPercentage(float xPercentage, float yPercentage)
    {
        // Normalize the x and y percentages to be between -1 and 1
        float normalizedX = (xPercentage / 100f) * 2f - 1f;
        float normalizedY = (yPercentage / 100f) * 2f - 1f;

        // Calculate the x and y positions based on the arena width and height
        float xPosition = normalizedX * (GetArenaWidth() / 2f);
        float yPosition = normalizedY * (GetArenaHeight() / 2f);

        // Return a Vector3 with the calculated x and y, and z set to 0
        return new Vector3(xPosition, yPosition, 0f);
    }
    public int GetSeedValue() => seedValue;

    #region // Game Size
    public float GetArenaWidth() => arenaWidth;
    public float GetArenaHeight() => arenaHeight;
    public float GetGameWidth() => gameWidth;
    public float GetGameHeight() => gameHeight;
    #endregion
    #region // References
    ...
    public WaymarkManager GetWaymarkManager() => waymarkManager;
...
//    
//    #region // Definitions
//    [SerializeField] private int currentFight;
//    [SerializeField] private int currentAttack;
//    [SerializeField] private int currentStep;
//    [SerializeField] private int currentAttackIndex;
//    #endregion
//    #region // Functions
//    public FightInfo(int fightEnum, int fightAttack = 0, int fightStep = 0)
//    {
//        InitializeFightInfo(fightEnum, fightAttack, fightStep);
//    }
//
//    // Expose these values via properties (optional)
//    public int CurrentFight => currentFight;
//    public int CurrentAttack => currentAttack;
//    public int CurrentStep => currentStep;
//    public int CurrentAttackIndex => currentAttackIndex;
//
//    public void InitializeFightInfo(int fightEnum,int fightAttack = 0, int fightStep = 0) 
//    {
//        SetCurrentFight(fightEnum);
//        SetCurrentAttack(fightAttack);
//        SetCurrentStep(fightStep);
//    }
//    public void SetCurrentFight(int newFight) { currentFight = newFight; }
...
//    public void GotoNextAttack() { currentAttackIndex++; }
